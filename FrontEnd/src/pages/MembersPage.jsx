...
export default function MembersPage() {
  ...
  // ── Get current user ID reliably from store OR localStorage ──────────────
  ...
  // ── Auto-refresh every 30s to pick up isOnline changes ───────────────────
  useEffect(() => {
    if (!activeWorkspace) return;
  ...
  // ── Mark the current user as online in the list ──────────────────────────
  // Even if Firestore hasn't updated yet, we know the current user is online
  const membersWithSelf = filtered.map(m => ({
    ...m,
    isOnline: m.userId === currentUid ? true : m.isOnline,
  }));

  const onlineCount = members.filter((m) =>
    m.isOnline || m.userId === currentUid
  ).length;
    ...
    <>
      <style>{`
        ...
        .mp-role-sel {
          ...
          font-family: inherit; transition: opacity 0.15s; border: 1px solid;
        }
        ...
        .mp-btn:hover:not(:disabled) { opacity: 0.9; transform: translateY(-1px); box-shadow: 0 6px 22px rgba(108,63,255,0.42); }
        ...
        .mp-row:hover { background: rgba(108,63,255,0.05); border-color: rgba(108,63,255,0.2); box-shadow: 0 4px 22px rgba(0,0,0,0.3); transform: translateY(-1px); }
        .mp-rm {
        ...
        .mp-rm:hover { background: rgba(243,107,107,0.12) !important; border-color: rgba(243,107,107,0.25) !important; color: #f36b6b !important; }
        @keyframes mp-spin { to { transform: rotate(360deg); } }
        .mp-spin { width: 14px; height: 14px; border: 2px solid rgba(255,255,255,0.3); border-top-color: #fff; border-radius: 50%; animation: mp-spin 0.7s linear infinite; flex-shrink: 0; }
        ...
        @keyframes mp-op { 0%,100% { box-shadow: 0 0 0 0 rgba(34,211,160,0.55); } 60% { box-shadow: 0 0 0 5px rgba(34,211,160,0); } }
        .mp-online { animation: mp-op 2.2s ease-in-out infinite; }
        @keyframes mp-online-dot { 0%,100%{opacity:1} 50%{opacity:0.5} }
        .mp-online-dot { animation: mp-online-dot 2s ease-in-out infinite; }
        .mp-search-wrap { position: relative; }
        .mp-search-icon { position: absolute; left: 11px; top: 50%; transform: translateY(-50%); font-size: 13px; color: #3a3a58; pointer-events: none; }
          ...
          <div style={S.headerBg}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 5 }}>
                  ...
                  <span style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: "#22d3a0" }}>
                    <span className="mp-online" style={{ width: 6, height: 6, borderRadius: "50%", background: "#22d3a0", display: "inline-block" }} />
                    {onlineCount} online
                  </span>
                  {/* Refresh indicator */}
                  <button
                    onClick={load}
                    style={{ background: "transparent", border: "none", cursor: "pointer", fontSize: 11, color: "#3a3a58", padding: "2px 6px", borderRadius: 6 }}
              ...
              <div className="mp-search-wrap">
                ...
                <input className="mp-input" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search members…" />
              </div>
            </div>
          </div>
          ...
          <div style={{ padding: "24px 32px", maxWidth: 860 }}>
            ...
            <div style={S.card}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                <div style={{ width: 30, height: 30, borderRadius: 9, background: "rgba(108,63,255,0.15)", border: "1px solid rgba(108,63,255,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>✉️</div>
              ...
              <form onSubmit={invite} style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "flex-end" }}>
                <div style={{ flex: 1, minWidth: 180 }}>
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="teammate@example.com" required className="mp-input" />
                </div>
                <div style={{ width: 148 }}>
              ...
              {error && (
                <div style={{ marginTop: 12, padding: "8px 12px", borderRadius: 8, background: "rgba(243,107,107,0.08)", border: "1px solid rgba(243,107,107,0.2)", display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "#f36b6b" }}>⚠ {error}</div>
              )}
              {success && (
                <div style={{ marginTop: 12, padding: "8px 12px", borderRadius: 8, background: "rgba(34,211,160,0.08)", border: "1px solid rgba(34,211,160,0.2)", display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "#22d3a0" }}>✓ {success}</div>
              )}

              <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px solid rgba(255,255,255,0.04)", display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
                <span style={S.sectionLabel}>Roles:</span>
                {Object.entries(ROLE_DESC).map(([r, desc]) => (
                  <div key={r} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: ROLE_COLOR[r].dot, display: "inline-block" }} />
            ...
            {loading ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[0, 1, 2].map((n) => (
                  <div key={n} className="mp-skel" style={{ ...S.row, gap: 14, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)" }}>
              ...
              <div style={{ textAlign: "center", padding: "60px 0" }}>
                ...
                <p style={S.mutedText}>{search ? "No members match your search" : "No members yet — invite your first teammate!"}</p>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                {membersWithSelf.map((m, i) => {
                  // ← fixed: use currentUid instead of user?.uid
                  const isMe  = m.userId === currentUid;
                  const rc    = ROLE_COLOR[m.role] || ROLE_COLOR.viewer;
                  const [c1, c2] = AVATAR_GRADS[i % AVATAR_GRADS.length];

                  return (
                    <div key={m.userId} className="mp-row mp-fu" style={{ animationDelay: `${i * 0.05}s` }}>

                      {/* Avatar with online dot */}
                      <div style={{ position: "relative", flexShrink: 0 }}>
                        <div style={{ width: 40, height: 40, borderRadius: "50%", background: `linear-gradient(135deg, ${c1}, ${c2})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", boxShadow: "0 2px 10px rgba(0,0,0,0.35)" }}>
                          {m.name?.slice(0, 2).toUpperCase() || "??"}
                        </div>
                        {/* ← online dot: green + pulsing if online, dark if offline */}
                        <div
                          className={m.isOnline ? "mp-online-dot" : ""}
                          style={{
                            position: "absolute", bottom: -1, right: -1,
                            width: 11, height: 11, borderRadius: "50%",
                            border: "2px solid #09090f",
                            background: m.isOnline ? "#22d3a0" : "#2a2a42",
                            boxShadow: m.isOnline ? "0 0 6px rgba(34,211,160,0.6)" : "none",
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <p style={{ ...S.bodyText, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.name}</p>
                          {isMe && (
                            <span style={{ fontSize: 9, fontWeight: 700, color: "#a78bfa", background: "rgba(108,63,255,0.15)", border: "1px solid rgba(108,63,255,0.25)", borderRadius: 20, padding: "1px 7px", flexShrink: 0 }}>YOU</span>
                          )}
                        </div>
                        <p style={{ ...S.subText, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.email}</p>
                      </div>

                      {/* Status — with glow when online */}
                      <div style={{ width: 80, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
                        <span style={{
                          width: 5, height: 5, borderRadius: "50%",
                          background: m.isOnline ? "#22d3a0" : "#2a2a42",
                          display: "inline-block", flexShrink: 0,
                          boxShadow: m.isOnline ? "0 0 5px rgba(34,211,160,0.7)" : "none",
                          transition: "all 0.3s",
                        }} />
                      ...
                      <div style={{ width: 110, display: "flex", justifyContent: "center" }}>
                        {isMe ? (
                          <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 12px", borderRadius: 20, border: "1px solid", color: rc.text, background: rc.bg, borderColor: rc.border, textTransform: "capitalize" }}>{m.role}</span>
                        ) : (
                          <select value={m.role} onChange={(e) => changeRole(m.userId, e.target.value)} className="mp-role-sel" style={{ color: rc.text, backgroundColor: rc.bg, borderColor: rc.border }}>
                            <option value="viewer">viewer</option>
                      ...
                      <div style={{ width: 28, display: "flex", justifyContent: "center" }}>
                        {!isMe && (
                          <button onClick={() => remove(m.userId, m.name)} title={`Remove ${m.name}`} className="mp-rm">✕</button>
                        )}
                      </div>
                    </div>
                  );
            ...
            {!loading && members.length > 0 && (
              <div style={{ display: "flex", alignItems: "center", gap: 20, marginTop: 20, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.04)" }}>
